...
# 1. Verify pipx is installed (crucial for modern Python environments)
if ! command -v pipx &> /dev/null; then
    echo -e "\033[1;31m[ERROR] pipx is not installed.\033[0m"
    echo "Please install it first (e.g., 'sudo pacman -S pipx' or 'sudo dnf install pipx')"
    exit 1
fi

# 2. Ensure the user's pipx binary path is available in their shell
pipx ensurepath > /dev/null 2>&1

# 3. Install the package globally straight from your GitHub repository!
# NOTE: Replace 'krishiv2489' if your GitHub username is different.
...
# 4. Run the Python auto-injector
